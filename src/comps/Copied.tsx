...
export interface CopiedProps<T extends object>
  extends React.SVGProps<SVGSVGElement>,
    SectionElementResult<T> {
  expandKey: string;
}

export const Copied = <T extends object, K extends TagType>(
  props: CopiedProps<T>,
) => {
  ...
  const click = (event: React.MouseEvent<SVGSVGElement, MouseEvent>) => {
    ...
    if (typeof value === 'number' && value === Infinity) {
      ...
      copyText = JSON.stringify(
        value,
        (_, v) => (typeof v === 'bigint' ? bigIntToString(v) : v),
        2,
      );
    }
  ...
  const child =
    isRender &&
    render({ ...elmProps, 'data-copied': copied } as React.HTMLAttributes<K>, {
      value,
      keyName,
      keys,
      parentValue,
