...
export type SymbolsElement<T extends TagType = 'span'> =
  SymbolsElementProps<T> & ComponentPropsWithoutRef<T>;
...
const reducer = (
  state: InitialState<TagType>,
  action: InitialState<TagType>,
) => ({
  ...state,
  ...action,
});
...
export const Symbols: FC<PropsWithChildren<SymbolsProps>> = ({
  initial,
  dispatch,
  children,
}) => {
  return (
    <Context.Provider value={initial}>
      <DispatchSymbols.Provider value={dispatch}>
        {children}
      </DispatchSymbols.Provider>
    </Context.Provider>
  );
};
