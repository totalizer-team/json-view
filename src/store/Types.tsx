...
export type TypesElement<T extends TagType> = TypesElementProps<T> &
...
const reducer = <T extends TagType>(
  state: InitialTypesState<T>,
  action: InitialTypesState<T>,
) => ({
  ...state,
  ...action,
});
...
export function Types<T extends TagType>({
  initial,
  dispatch,
  children,
}: PropsWithChildren<TypesProps<T>>) {
  return (
    <Context.Provider value={initial as unknown as InitialTypesState<TagType>}>
      <DispatchTypes.Provider value={dispatch}>
        {children}
      </DispatchTypes.Provider>
    </Context.Provider>
  );
}
